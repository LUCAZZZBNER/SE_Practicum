...
import static com.delivery.media.ControllerTestSupport.merchantPrincipal;
import static com.delivery.media.ControllerTestSupport.successfulDataId;
import static com.delivery.media.ControllerTestSupport.unauthenticated;
...
import com.delivery.media.item.image.controller.ImageController;
...
class ImageControllerTests {
  ...
  void uploadForwardsMerchantAndMultipartFile() throws Exception {
    when(service.upload(any(Long.class), any()))
        .thenReturn(
    ...
    MockMultipartFile file =
        new MockMultipartFile("file", "product.webp", "image/webp", new byte[] {1, 2, 3, 4});
    mvc.perform(
            multipart("/api/v1/files/images")
                .file(file)
                .requestAttr("currentPrincipal", merchantPrincipal(2)))
        .andExpect(status().isCreated())
        .andExpect(successfulDataId(301));
  ...
  void uploadRejectsMissingAuthentication() throws Exception {
    MockMultipartFile file =
        new MockMultipartFile("file", "product.jpg", "image/jpeg", new byte[] {1});
