package com.delivery.order.merchant.shop.service.impl;
...
import com.delivery.order.merchant.shop.dao.RestaurantDao;
import com.delivery.order.merchant.shop.entity.ShopEntity;
...
public class RestaurantServiceImpl implements RestaurantService {
  ...
  public PageResult<ShopView> list(ListQuery query) {
    ...
    List<ShopView> items =
        restaurantDao
            .list(mine, query.merchantId(), keyword, status, sortBy, sortOrder, pageSize, offset)
            .stream()
            .map(RestaurantServiceImpl::toView)
            .toList();
  ...
  public ShopView update(long merchantId, long shopId, UpdateRequest request) {
    ...
    if (OPEN.equals(status)
        && (shop.getAddressRegion() == null
            || shop.getAddressRegion().isBlank()
            || shop.getAddressDetail() == null
            || shop.getAddressDetail().isBlank()
            || shop.getAddressPhone() == null
            || shop.getAddressPhone().isBlank())) {
  ...
  public ShopView updateAddress(long merchantId, long shopId, AddressRequest request) {
    ...
    if (request == null
        ...
        || request.detail() == null
        || request.detail().isBlank()
        || request.phone() == null
        || !request.phone().matches("1[0-9]{10}|0[0-9]{10,11}")) {
      throw new BusinessException(ApiError.VALIDATION_ERROR);
    }
    restaurantDao.updateAddress(
        shopId, request.region().trim(), request.detail().trim(), request.phone().trim());
    return toView(requireById(shopId));
  }
  ...
  private static ShopView toView(ShopEntity shop) {
    return new ShopView(
        ...
        shop.getDescription(),
        shop.getStatus(),
        shop.getAddressRegion(),
        shop.getAddressDetail(),
        shop.getAddressPhone(),
        shop.getCreatedAt(),
        shop.getUpdatedAt());
  }
