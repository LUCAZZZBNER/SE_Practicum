package com.delivery.order.order;

import static com.delivery.order.ControllerTestSupport.JSON;
import static com.delivery.order.ControllerTestSupport.merchantPrincipal;
import static com.delivery.order.ControllerTestSupport.successfulDataId;
import static com.delivery.order.ControllerTestSupport.successfulPage;
import static com.delivery.order.ControllerTestSupport.unauthenticated;
import static com.delivery.order.ControllerTestSupport.userPrincipal;
import static com.delivery.order.ControllerTestSupport.validationError;
import static com.delivery.order.ControllerTestSupport.withApiErrors;
import static com.delivery.order.TestFixtures.order;
import static com.delivery.order.TestFixtures.orderPage;
...
import com.delivery.order.order.controller.OrderController;
...
class OrderControllerTests {
  ...
  void createForwardsSkuVersionAddressAndRemarkSentByCartView() throws Exception {
    ...
    mvc.perform(
            post("/api/v1/orders")
                .requestAttr("currentPrincipal", userPrincipal(7))
                .header("X-Idempotency-Key", "checkout-key")
                .contentType(JSON)
                .content(
                    """
    ...
    verify(service)
        .create(
            ...
            argThat(
                request ->
                    request.addressId() == 51
                        && request.items().get(0).skuVersion() == 3
                        && request.remark().equals("少放辣椒")));
  }
  ...
  void actionEndpointsForwardIdempotencyKeysUsedByOrderViews() throws Exception {
    ...
    mvc.perform(
            post("/api/v1/orders/40/pay")
                .requestAttr("currentPrincipal", userPrincipal(7))
                .header("X-Idempotency-Key", "pay-key")
                .contentType(JSON)
                .content("{}"))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(40));
    mvc.perform(
            post("/api/v1/orders/40/confirm-receipt")
                .requestAttr("currentPrincipal", userPrincipal(7))
                .header("X-Idempotency-Key", "receipt-key")
                .contentType(JSON)
                .content("{}"))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(40));
    mvc.perform(
            post("/api/v1/merchant/orders/40/prepare")
                .requestAttr("currentPrincipal", merchantPrincipal(2))
                .header("X-Idempotency-Key", "prepare-key")
                .contentType(JSON)
                .content("{}"))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(40));
    mvc.perform(
            post("/api/v1/merchant/orders/40/deliver")
                .requestAttr("currentPrincipal", merchantPrincipal(2))
                .header("X-Idempotency-Key", "deliver-key")
                .contentType(JSON)
                .content("{}"))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(40));
  ...
  void refundLookupForwardsUserOwnershipAndOrderId() throws Exception {
    when(service.getRefund(7, 40))
        .thenReturn(
            new OrderService.RefundView(
                40, "REFUND-40", new java.math.BigDecimal("12.50"), "REFUNDED"));
    mvc.perform(get("/api/v1/orders/40/refund").requestAttr("currentPrincipal", userPrincipal(7)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.orderId").value(40));
  ...
  void userListForwardsEveryFilterPagingAndSortInput() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/orders")
                ...
                .queryParam("sortBy", "total")
                .queryParam("sortOrder", "desc"))
        .andExpect(status().isOk())
        .andExpect(successfulPage(1, 10, 1));
    verify(service)
        .listMine(7, new OrderService.ListQuery("PENDING_PAYMENT", 1, 100, "total", "desc"));
  }
  ...
  void userGetAndCancelForwardOwnershipAndOrderId() throws Exception {
    ...
    mvc.perform(get("/api/v1/orders/40").requestAttr("currentPrincipal", userPrincipal(7)))
        ...
        .andExpect(successfulDataId(40));
    mvc.perform(
            post("/api/v1/orders/40/cancel")
                .requestAttr("currentPrincipal", userPrincipal(7))
                .header("X-Idempotency-Key", "cancel-key")
                .contentType(JSON)
                .content("{}"))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(40));
  ...
  void merchantListForwardsAbsentAndFullySpecifiedQueries() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/merchant/orders").requestAttr("currentPrincipal", merchantPrincipal(2)))
        .andExpect(status().isOk())
        .andExpect(successfulPage(1, 10, 1));
    verify(service)
        .listMerchantOrders(
            2, new OrderService.MerchantListQuery(null, null, null, null, null, null));
    ...
    verify(service)
        .listMerchantOrders(
            2, new OrderService.MerchantListQuery(10L, "PAID", 1, 100, "createdAt", "asc"));
  }
  ...
  void merchantDetailForwardsMerchantOwnershipAndOrderId() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/merchant/orders/40").requestAttr("currentPrincipal", merchantPrincipal(2)))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(40));
  ...
  @Test
  void orderQueriesRejectMalformedNumbersAndProtectedEndpointsRejectMissingPrincipal()
      throws Exception {
    mvc.perform(
            get("/api/v1/orders")
                .requestAttr("currentPrincipal", userPrincipal(7))
  ...
  void detailAndCancelRejectNonPositiveOrderIds() throws Exception {
    ...
    mvc.perform(post("/api/v1/orders/-1/cancel").requestAttr("currentPrincipal", userPrincipal(7)))
        ...
        .andExpect(validationError());
    mvc.perform(
            get("/api/v1/merchant/orders/0").requestAttr("currentPrincipal", merchantPrincipal(2)))
        .andExpect(status().isBadRequest())
        .andExpect(validationError());
    verifyNoInteractions(service);
  }
