...
public class ShoppingServiceImpl implements ShoppingService {
  ...
  public CartView getCart(long userId) {
    ...
    List<CartItemView> items =
        shoppingDao.listByUser(userId).stream().map(ShoppingServiceImpl::toView).toList();
    BigDecimal total =
        ...
        .map(
            item -> {
              ...
              return new CheckoutItem(
                  ...
                  item.getSkuId() == null ? 0 : item.getSkuId(),
                  item.getSkuVersion() == null ? item.getProductVersion() : item.getSkuVersion());
            })
        .toList();
  }
  ...
  public void prepareForCheckout(long userId, List<Long> cartItemIds, String checkoutId) {
    userService.requireActive(userId);
    List<Long> ids = normalizeIds(cartItemIds);
    if (checkoutId == null || checkoutId.isBlank()) throw new BusinessException(ApiError.VALIDATION_ERROR);
    if (shoppingDao.claimSelected(userId, ids, checkoutId) != ids.size()) throw new BusinessException(ApiError.CART_EMPTY);
  }
  ...
  public void removeAfterCheckout(long userId, List<Long> cartItemIds, String checkoutId) {
    userService.requireActive(userId);
    List<Long> ids = normalizeIds(cartItemIds);
    shoppingDao.deleteSelected(userId, ids, checkoutId);
  }
  ...
  public void releaseAfterCheckout(long userId, List<Long> cartItemIds, String checkoutId) {
    userService.requireActive(userId);
    shoppingDao.releaseSelected(userId, normalizeIds(cartItemIds), checkoutId);
  }
  ...
  private static void ensureAvailable(CartItemEntity item) {
    ...
    ensureStock(
        item.getQuantity(),
        item.getSkuStock() == null ? item.getProductStock() : item.getSkuStock());
  }
  ...
  private ItemService.ProductView requireCartProduct(long productId) {
    try {
      ...
      if (exception.error() == ApiError.RESOURCE_NOT_FOUND)
        throw new BusinessException(ApiError.PRODUCT_OFF_SALE);
      throw exception;
    }
  }
