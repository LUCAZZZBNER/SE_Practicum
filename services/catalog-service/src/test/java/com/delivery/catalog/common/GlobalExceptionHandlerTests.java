package com.delivery.catalog.common;
...
class GlobalExceptionHandlerTests {
  ...
  void everyDocumentedBusinessErrorPreservesItsHttpStatusCodeMessageAndData(ApiError error) {
    ...
    ResponseEntity<ApiResponse<Object>> response =
        handler.businessFailure(new BusinessException(error, data));

    assertThat(response.getStatusCode()).isEqualTo(error.status());
    assertThat(response.getBody())
        .isEqualTo(new ApiResponse<>(error.code(), error.message(), data));
  }

  @Test
  void unexpectedExceptionsAreSanitized() {
    ResponseEntity<ApiResponse<Object>> response =
        handler.unexpectedFailure(new RuntimeException("password=secret; database details"));
