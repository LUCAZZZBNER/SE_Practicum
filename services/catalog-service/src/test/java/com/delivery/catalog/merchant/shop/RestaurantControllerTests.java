package com.delivery.catalog.merchant.shop;
...
import static com.delivery.catalog.TestFixtures.shop;
import static com.delivery.catalog.TestFixtures.shopPage;
...
import com.delivery.catalog.merchant.shop.controller.RestaurantController;
...
class RestaurantControllerTests {
  ...
  void publicListForwardsAbsentOptionalParametersAsNull() throws Exception {
    ...
    mvc.perform(get("/api/v1/shops"))
        .andExpect(status().isOk())
        .andExpect(successfulPage(1, 10, 1));
    verify(service)
        .list(new RestaurantService.ListQuery(null, null, null, null, null, null, null, null));
  }
  ...
  void merchantListForwardsEveryFilterSortAndIdentityInput() throws Exception {
    ...
    mvc.perform(
        ...
        .andExpect(successfulPage(1, 10, 1));
    verify(service)
        .list(new RestaurantService.ListQuery(1, 100, "rice", "OPEN", true, "name", "asc", 2L));
    ...
    mvc.perform(
        ...
        .andExpect(status().isOk());
    verify(service)
        .list(new RestaurantService.ListQuery(null, null, null, null, true, null, null, null));
  }
  ...
  void updateAddressForwardsMerchantShopAndAddressFields() throws Exception {
    ...
    mvc.perform(
            patch("/api/v1/shops/10/address")
                .requestAttr("currentPrincipal", merchantPrincipal(2))
                .contentType(JSON)
                .content("{\"region\":\"杭州\",\"detail\":\"学院路\",\"phone\":\"05710000000\"}"))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(10));
  ...
  void shopAddressUsesTheFieldsReadByTheFrontend() throws Exception {
    when(service.get(10))
        .thenReturn(
            new RestaurantService.ShopView(
                10, 2, "Shop", null, "CLOSED", "杭州", "学院路", "05710000000", null, null));
