...
import static com.delivery.catalog.ControllerTestSupport.JSON;
import static com.delivery.catalog.ControllerTestSupport.merchantPrincipal;
import static com.delivery.catalog.ControllerTestSupport.successfulDataId;
import static com.delivery.catalog.ControllerTestSupport.successfulEnvelope;
import static com.delivery.catalog.ControllerTestSupport.successfulPage;
import static com.delivery.catalog.ControllerTestSupport.unauthenticated;
import static com.delivery.catalog.ControllerTestSupport.userPrincipal;
import static com.delivery.catalog.ControllerTestSupport.validationError;
import static com.delivery.catalog.ControllerTestSupport.withApiErrors;
import static com.delivery.catalog.TestFixtures.category;
import static com.delivery.catalog.TestFixtures.product;
import static com.delivery.catalog.TestFixtures.productPage;
...
import com.delivery.catalog.item.controller.ItemController;
...
class ItemControllerTests {
  ...
  void skuStatusToggleAcceptsOnlyStatusAndVersionFromMerchantProductsView() throws Exception {
    when(skuService.update(any(Long.class), any(Long.class), any()))
        .thenReturn(
            new SkuService.SkuView(
                1001, 11, "Large", new BigDecimal("12.50"), 4, "ON_SALE", 4, null, null));

    mvc.perform(
            patch("/api/v1/skus/1001")
                .requestAttr("currentPrincipal", merchantPrincipal(2))
                .contentType(JSON)
                .content("{\"status\":\"ON_SALE\",\"version\":3}"))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(1001));
    verify(skuService)
        .update(
            eq(2L),
            eq(1001L),
  ...
  void categoryEndpointsForwardEveryRequestAndReturnServiceOutput() throws Exception {
    ...
    mvc.perform(
            post("/api/v1/shops/10/categories")
                .requestAttr("currentPrincipal", merchantPrincipal(2))
                .contentType(JSON)
                .content("{\"name\":\"Meals\",\"sortOrder\":0}"))
        .andExpect(status().isCreated())
        .andExpect(successfulDataId(21));
    mvc.perform(get("/api/v1/shops/10/categories"))
        .andExpect(status().isOk())
        .andExpect(successfulEnvelope())
        .andExpect(jsonPath("$.data[0].id").value(10));
    mvc.perform(
            patch("/api/v1/categories/21")
                .requestAttr("currentPrincipal", merchantPrincipal(2))
                .contentType(JSON)
        ...
        .andExpect(successfulDataId(21));
    mvc.perform(
            delete("/api/v1/categories/21").requestAttr("currentPrincipal", merchantPrincipal(2)))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(21));
  ...
  void productListForwardsAbsentAndFullySpecifiedQueries() throws Exception {
    ...
    mvc.perform(get("/api/v1/shops/10/products"))
        .andExpect(status().isOk())
        .andExpect(successfulPage(1, 10, 1));
    verify(service)
        .listProducts(
            10, new ItemService.ProductQuery(null, null, null, null, null, null, null, null));

    mvc.perform(
            get("/api/v1/shops/10/products")
                .requestAttr("currentPrincipal", merchantPrincipal(2))
    ...
    verify(service)
        .listProducts(
            10, new ItemService.ProductQuery(21L, "rice", 1, 100, "price", "desc", true, 2L));

    mvc.perform(
            get("/api/v1/shops/10/products")
                .requestAttr("currentPrincipal", userPrincipal(7))
                .queryParam("includeOffSale", "true"))
        .andExpect(status().isOk());
    verify(service)
        .listProducts(
            10, new ItemService.ProductQuery(null, null, null, null, null, null, true, null));
  }
  ...
  void productGetDefaultsOffSaleToFalseAndAcceptsTrue() throws Exception {
    when(service.getProduct(
            any(Long.class),
            org.mockito.ArgumentMatchers.anyBoolean(),
            org.mockito.ArgumentMatchers.nullable(Long.class)))
        .thenReturn(product(30));
    mvc.perform(get("/api/v1/products/30"))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(30));
    mvc.perform(
  ...
  void updateProductForwardsAllOptionalFieldsIncludingVersion() throws Exception {
    ...
    mvc.perform(
            patch("/api/v1/products/30")
                .requestAttr("currentPrincipal", merchantPrincipal(2))
                .contentType(JSON)
                .content(
                    """
                    {"categoryId":22,"name":"Noodles","description":null,
                     "price":12.50,"stock":5,"status":"ON_SALE","version":3}
                    """))
        .andExpect(status().isOk())
        .andExpect(successfulDataId(30));
    verify(service)
        .updateProduct(
            ...
            argThat(
                request ->
                    request.categoryId() == 22L
                        && request.name().equals("Noodles")
                        && request.description() == null
                        && request.price().equals(new BigDecimal("12.50"))
                        && request.stock() == 5
                        && request.status().equals("ON_SALE")
                        && request.version() == 3L));
  }
  ...
  void merchantMutationsRejectMissingPrincipal() throws Exception {
    mvc.perform(
            post("/api/v1/shops/10/categories").contentType(JSON).content("{\"name\":\"Meals\"}"))
    ...
    mvc.perform(
            post("/api/v1/products")
                .contentType(JSON)
                .content(
                    "{\"shopId\":10,\"categoryId\":21,\"name\":\"Rice\",\"price\":1,\"stock\":0}"))
        .andExpect(status().isUnauthorized())
        .andExpect(unauthenticated());
