...
import com.delivery.catalog.common.DeleteResult;
...
import com.delivery.catalog.item.dao.SkuDao;
import com.delivery.catalog.item.entity.CategoryEntity;
...
public class ItemServiceImpl implements ItemService {
  ...
  public CategoryView updateCategory(
      long merchantId, long categoryId, UpdateCategoryRequest request) {
    ...
    try {
      itemDao.updateCategory(
          categoryId, request.isNameSpecified(), name, request.isSortOrderSpecified(), sortOrder);
    } catch (DuplicateKeyException exception) {
      throw new BusinessException(ApiError.RESOURCE_CONFLICT);
    }
  ...
  public ProductView createProduct(long merchantId, CreateProductRequest request) {
    ...
    if (price == null) {
      price =
          skus.stream()
              .map(SkuService.CreateRequest::price)
              .min(BigDecimal::compareTo)
              .orElseThrow();
    }
    if (request.stock() == null)
      stock = skus.stream().mapToInt(SkuService.CreateRequest::stock).sum();
    validatePrice(price);
    if (stock < 0) throw new BusinessException(ApiError.VALIDATION_ERROR);
    ...
    if (skuDao != null)
      for (SkuService.CreateRequest requestSku : skus) {
        var sku = new com.delivery.catalog.item.entity.SkuEntity();
        sku.setProductId(product.getId());
        sku.setName(requestSku.name().trim());
        sku.setPrice(requestSku.price());
    ...
    List<ProductView> items =
        itemDao
            .listProducts(
                ...
                offset)
            .stream()
            .map(product -> toProductView(product, includeOffSale))
            .toList();
  ...
  public ProductView getProduct(long productId, boolean includeOffSale, Long merchantId) {
    ...
    if (!includeOffSale
        && view.skus().stream().noneMatch(sku -> ON_SALE.equals(sku.status()) && sku.stock() > 0)) {
      throw new BusinessException(ApiError.RESOURCE_NOT_FOUND);
    }
    return view;
  }
  ...
  public ProductView updateProduct(long merchantId, long productId, UpdateProductRequest request) {
    ...
    int updated =
        itemDao.updateProduct(
            productId,
            expectedVersion,
            request.isCategoryIdSpecified(),
            request.categoryId(),
            request.isNameSpecified(),
            name,
            request.isDescriptionSpecified(),
            request.description(),
            request.isPriceSpecified(),
            request.price(),
            request.isStockSpecified(),
            request.stock(),
            request.isStatusSpecified(),
            status,
            request.isImageIdSpecified(),
            request.imageId());
    if (updated != 1) {
      throw new BusinessException(ApiError.RESOURCE_CONFLICT);
    }
  ...
  public void restoreStock(List<StockRestore> restorations) {
    ...
    for (StockRestore restoration : restorations) {
      ...
      if (restoration.reservationId() != null && !restoration.reservationId().isBlank()) {
        int claimed = stockReservationDao.claimRestore(restoration.reservationId(), restoration.productId(), restoration.skuId());
        if (claimed == 0) continue;
      }
      int restored =
          restoration.skuId() > 0
              ? skuDao.restoreStock(restoration.skuId(), restoration.quantity())
  ...
  @Override
  @Transactional
  public void confirmReservations(List<String> reservationIds) {
    if (reservationIds == null) throw new BusinessException(ApiError.VALIDATION_ERROR);
    for (String reservationId : reservationIds) {
      if (reservationId == null || reservationId.isBlank()) throw new BusinessException(ApiError.VALIDATION_ERROR);
      stockReservationDao.confirm(reservationId);
    }
  }
  ...
  private ProductSnapshot reserveOne(ReservationRequest request) {
    ...
    if (request.reservationId() != null && !request.reservationId().isBlank()) {
      Integer reserved = stockReservationDao.findQuantity(request.reservationId(), request.productId(), request.skuId());
      if (reserved != null) {
        if (request.skuId() > 0) {
          var sku = skuDao.findById(request.skuId());
          return new ProductSnapshot(product.getId(), product.getShopId(), product.getName(), sku.getPrice(), reserved,
              sku.getVersion(), sku.getId(), sku.getName(), product.getImageUrl());
        }
        return new ProductSnapshot(product.getId(), product.getShopId(), product.getName(), product.getPrice(), reserved,
            product.getVersion(), 0, null, product.getImageUrl());
      }
    }
    if (request.skuId() > 0) {
      var sku = skuDao.findById(request.skuId());
      ...
      if (sku.getVersion() != request.expectedVersion())
      ...
      if (skuDao.reserveStock(sku.getId(), request.expectedVersion(), request.quantity()) != 1)
        throw new BusinessException(ApiError.INSUFFICIENT_STOCK);
      if (request.reservationId() != null && !request.reservationId().isBlank())
        stockReservationDao.insert(request.reservationId(), request.productId(), request.skuId(), request.quantity());
    ...
    if (request.reservationId() != null && !request.reservationId().isBlank())
      stockReservationDao.insert(request.reservationId(), request.productId(), 0, request.quantity());
    return new ProductSnapshot(
        product.getId(),
        product.getShopId(),
        product.getName(),
  ...
  private ProductView toProductView(ProductEntity product, boolean includeOffSale) {
    List<SkuService.SkuView> skus =
        skuDao == null
            ? List.of()
            : skuDao.listByProduct(product.getId()).stream()
                .filter(s -> includeOffSale || ON_SALE.equals(s.getStatus()))
                .map(
                    s ->
                        new SkuService.SkuView(
                            s.getId(),
                            s.getProductId(),
                            s.getName(),
                            s.getPrice(),
