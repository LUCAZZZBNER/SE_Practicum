package com.delivery.identity.user;

import static com.delivery.identity.ControllerTestSupport.JSON;
import static com.delivery.identity.ControllerTestSupport.successfulDataId;
import static com.delivery.identity.ControllerTestSupport.successfulEnvelope;
import static com.delivery.identity.ControllerTestSupport.unauthenticated;
import static com.delivery.identity.ControllerTestSupport.userPrincipal;
import static com.delivery.identity.ControllerTestSupport.validationError;
import static com.delivery.identity.ControllerTestSupport.withApiErrors;
import static com.delivery.identity.TestFixtures.user;
import static com.delivery.identity.TestFixtures.userSession;
...
import com.delivery.identity.user.controller.UserController;
...
class UserControllerTests {
            ...
            post("/api/v1/users")
                ...
                .content(
                    ...
                    {"account":"alice","password":"secret","passwordConfirm":"secret",
                     "nickname":"Alice","phone":"13800000000"}
                    """))
        .andExpect(status().isCreated())
        .andExpect(successfulDataId(1));

    ArgumentCaptor<UserService.RegisterRequest> request =
        ArgumentCaptor.forClass(UserService.RegisterRequest.class);
    verify(service).register(request.capture());
    org.assertj.core.api.Assertions.assertThat(request.getValue())
        .isEqualTo(
            new UserService.RegisterRequest("alice", "secret", "secret", "Alice", "13800000000"));
  }
  ...
  @ValueSource(
      strings = {
        ...
        "{\"account\":\"alice\",\"password\":\"secret\",\"passwordConfirm\":\"secret\"}",
        "{\"account\":\" \""
            + " ,\"password\":\"secret\",\"passwordConfirm\":\"secret\",\"nickname\":\"Alice\"}",
        "{\"account\":\"alice\",\"password\":\""
            + " \",\"passwordConfirm\":\"secret\",\"nickname\":\"Alice\"}",
        "{\"account\":\"alice\",\"password\":\"secret\",\"passwordConfirm\":\""
            + " \",\"nickname\":\"Alice\"}",
        "{\"account\":\"alice\",\"password\":\"secret\",\"passwordConfirm\":\"secret\",\"nickname\":\""
            + " \",\"phone\":\"138\"}",
        "{\"account\":\"alice\",\"password\":\"secret\",\"passwordConfirm\":\"secret\","
