...
<template>
  ...
  <div v-else class="order-list">
    <article
      v-for="row in orders"
      :key="row.id"
      :data-testid="`merchant-order-card-${row.id}`"
      class="order-row"
    >
      <div class="order-copy">
        ...
        <div class="transaction-status">
          <el-tag :type="getPaymentStatus(row.paymentStatus).type" effect="plain">
            {{ getPaymentStatus(row.paymentStatus).text }}
          </el-tag>
          <el-tag :type="getRefundStatus(row.refundStatus).type" effect="plain">
            {{ getRefundStatus(row.refundStatus).text }}
          </el-tag>
        </div>
...
.order-heading, .transaction-status { display: flex; align-items: center; gap: 10px; }
...
@media (max-width: 768px) {
  .order-row { align-items: stretch; flex-direction: column; gap: 14px; padding: 14px; }
  .order-heading, .transaction-status { align-items: flex-start; flex-wrap: wrap; }
  .shop-name { white-space: normal; overflow-wrap: anywhere; }
