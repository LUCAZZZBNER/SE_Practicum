# 轻量级外卖平台数据库 V1 记录与 V2 对齐说明

> [!WARNING]
> 本文主体记录已经执行的 V1 初始结构，不再决定业务范围，也不是阶段 1 最终模式。当前业务以 `docs/software-requirements-specification.md` 为准，HTTP/字段以 `docs/api/backend-api-design.md` 为准，实施步骤以根目录 `ABC阶段1业务TDD开发与联调执行文档.md` 为准。V1 与当前契约的差异必须通过 `V2__align_schema_with_api_contract.sql` 修正，禁止修改已发布 V1。旧段落中排除商家独立账号、分类逻辑删除/排序、商品乐观锁、订单幂等/取消的结论全部作废。
...
> 更新日期：2026-09-04
>
> 当前状态：V1 已在开发库和测试库通过 Flyway 执行；V1 不是最终业务模式，下一步新增 V2 对齐 SRS/API

## 1. 文档目的和效力

本文档只定义课程阶段 1 必须使用的数据库结构，是 B 编写 Flyway V1、Entity、Mapper 和 ServiceImpl 的依据。

执行范围以 SRS 和 API 文档为准。阶段 1 必须实现商家独立账号、分类排序/逻辑删除、商品乐观锁、订单幂等和取消；支付、退款、骑手和配送调度仍不进入阶段 1。
...
## 4. V1 已执行表结构（历史事实，不是最终业务结构）

本节逐字段记录 V1 实际创建了什么，便于编写 V2；其中的商家依附用户、单商家单店、分类无排序/删除、商品无 version、订单无幂等/取消等限制不得继续作为业务规则。目标结构以本文顶部的来源优先级和新执行文档第 7.4 节为准。
...
V1 尚未包含分类删除和排序字段；阶段 1 必须由 V2 增加并由 ItemServiceImpl 实现。
...
- `shop_id REFERENCES shops(id)`；
- `total_amount > 0`；
- V1 只允许 `status = 'PENDING_PAYMENT'`，V2 必须扩展当前契约需要的状态；
...
- V1 尚未实现取消；V2 和 OrderServiceImpl 必须实现 `PENDING_PAYMENT → CANCELLED`，真实支付、退款和配送仍不实现；
- 只能查询本人的订单。
...
完整 V1 以仓库中的 `backend/src/main/resources/db/migration/V1__create_core_tables.sql` 为准；后续只新增迁移。

## 9. V1 未包含的内容和 V2 处理结论

以下内容没有出现在 V1，但根据当前 SRS/API，必须进入 V2 和阶段 1 测试：
...
- 图片表、地址表、支付表、日志表；
- 为阶段 2 猜测的预留字段；
- 超出 API 白名单的复杂分页、排序和性能优化。

V2 只补齐当前 SRS/API 和现有契约需要的字段、约束和索引，不添加阶段 2 猜测能力。
...
- [x] 密码字段只保存摘要；
- [x] 订单明细保存名称和单价快照；
- [x] 已如实记录 V1 不含幂等、取消和乐观锁字段；这些缺口由 V2 修正。

### 10.2 关系和约束

- [x] account 唯一；
- [x] 已记录 V1 的商家依附用户约束；V2 改为独立商家账号。
- [x] 已记录 V1 的单商家单店约束；V2 改为同一商家店名唯一。
- [x] 同店分类名唯一；
- [x] 同用户同商品购物车项唯一；
- [x] 必要外键均为 RESTRICT；
