...
1. `backend/src/main/resources/db/migration/V1__create_core_tables.sql` 至 `V10__harden_v12_persistence.sql`：数据库结构的唯一执行来源；
2. `backend/src/main/resources/mapper/**/*.xml`：实际 SQL 读写、查询条件和索引使用场景；
3. `backend/src/main/java/**/entity`、`**/dao`、`**/service/impl`：字段映射、事务、状态流转和数据规则；
...
- 开发库默认 `delivery_dev`，测试库使用 `delivery_test`。

## 10. 安全和隐私

- 用户、商家密码只存哈希，API 响应和日志不得返回 `password_hash`；
- 数据库账号和密码来自环境变量 `DELIVERY_DB_USERNAME`、`DELIVERY_DB_PASSWORD`，不得提交真实 `.env`；
