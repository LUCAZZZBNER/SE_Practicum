# [归档] B：业务开发前数据库准备逐步执行手册

> [!WARNING]
> 本文件已经归档，只用于证明 B 曾经怎样完成 MySQL、Flyway V1 和本机环境准备，不再作为业务开发或数据库最终设计依据。当前阶段请执行根目录 [ABC 阶段 1 业务 TDD 开发与联调执行文档](ABC阶段1业务TDD开发与联调执行文档.md)；业务范围以 SRS 为准，HTTP 契约以 API 文档为准。本文后续任何“等待 A”“22 个接口”“共用登录”“HttpSession”或排除幂等、取消、乐观锁的旧结论均已作废。V1 已发布且不得修改，差异统一通过 V2 修正。
