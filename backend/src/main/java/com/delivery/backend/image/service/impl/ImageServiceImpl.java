...
@Service public class ImageServiceImpl implements ImageService {
 ...
 @Override public ImageView upload(long merchantId,MultipartFile file){merchants.requireActive(merchantId); if(file==null||file.isEmpty()||file.getSize()>MAX_SIZE) throw new BusinessException(ApiError.IMAGE_INVALID); String type=file.getContentType(); if(!TYPES.contains(type)) throw new BusinessException(ApiError.IMAGE_INVALID); try {byte[] bytes=file.getBytes(); if(bytes.length<4) throw new BusinessException(ApiError.IMAGE_INVALID); if(type.equals("image/jpeg") && !(bytes[0]==(byte)0xff&&bytes[1]==(byte)0xd8)) throw new BusinessException(ApiError.IMAGE_INVALID); if(type.equals("image/png") && !(bytes[0]==(byte)0x89&&bytes[1]=='P'&&bytes[2]=='N'&&bytes[3]=='G')) throw new BusinessException(ApiError.IMAGE_INVALID); if(type.equals("image/webp") && !(bytes[0]=='R'&&bytes[1]=='I'&&bytes[2]=='F'&&bytes[3]=='F'&&bytes.length>11&&bytes[8]=='W'&&bytes[9]=='E'&&bytes[10]=='B'&&bytes[11]=='P')) throw new BusinessException(ApiError.IMAGE_INVALID); ImageEntity e=new ImageEntity(); e.setContentType(type);e.setSize(file.getSize());e.setContent(bytes);e.setUrl("/uploads/products/pending-"+System.nanoTime());dao.insert(e);e.setUrl("/uploads/products/"+e.getId());dao.updateUrl(e.getId(),e.getUrl());return toView(e);} catch(IOException ex){throw new BusinessException(ApiError.IMAGE_INVALID);} }
