package com.delivery.backend.item.image.controller;
...
public class ImageController {
  ...
  public ResponseEntity<ApiResponse<ImageService.ImageView>> upload(
      @RequestAttribute("currentPrincipal") CurrentPrincipal p,
      @RequestPart("file") MultipartFile file) {
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(ApiResponse.success(service.upload(p.id(), file)));
  }
