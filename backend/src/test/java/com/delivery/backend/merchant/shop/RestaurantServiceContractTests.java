package com.delivery.backend.merchant.shop;
...
class RestaurantServiceContractTests extends ServiceContractTestSupport {
  ...
  void createDefaultsToClosedAndRejectsDuplicateNameForTheSameMerchant() {
    ...
    RestaurantService.ShopView shop =
        service.create(merchantId, new RestaurantService.CreateRequest("Rice Shop", null));
    assertThat(shop.id()).isPositive();
    assertThat(shop.merchantId()).isEqualTo(merchantId);
    assertThat(shop.status()).isEqualTo("CLOSED");
    assertBusinessError(
        ApiError.RESOURCE_CONFLICT,
        () ->
            service.create(
                merchantId, new RestaurantService.CreateRequest("Rice Shop", "duplicate")));
  }

  @Test
  void publicAndMineListsApplyPagingFilteringAndOwnership() {
    long merchantId = merchant("shop-owner-b").id();
    RestaurantService.ShopView closed =
        service.create(merchantId, new RestaurantService.CreateRequest("Noodle Shop", null));

    PageResult<RestaurantService.ShopView> mine =
        service.list(
            new RestaurantService.ListQuery(
                1, 10, "Noodle", "CLOSED", true, "name", "asc", merchantId));
    ...
    PageResult<RestaurantService.ShopView> publiclyVisible =
        service.list(new RestaurantService.ListQuery(1, 10, null, null, false, null, null, null));
    assertThat(publiclyVisible.items()).allMatch(shop -> shop.status().equals("OPEN"));

    PageResult<RestaurantService.ShopView> closedFilter =
        service.list(
            new RestaurantService.ListQuery(1, 10, "Noodle", "CLOSED", false, null, null, null));
    assertThat(closedFilter.items())
        .extracting(RestaurantService.ShopView::id)
  ...
  void onlyTheOwnerCanUpdateAndRequireOwnedReturnsTheSnapshot() {
    ...
    RestaurantService.ShopView shop =
        service.create(ownerId, new RestaurantService.CreateRequest("Owner Shop", null));
  ...
  void onlyOpenShopsAreOrderable() {
    ...
    RestaurantService.ShopView shop =
        service.create(merchantId, new RestaurantService.CreateRequest("Orderable Shop", null));
    ...
    open.setStatus("OPEN");
    assertBusinessError(
        ApiError.SHOP_ADDRESS_REQUIRED, () -> service.update(merchantId, shop.id(), open));
