...
class ItemServiceContractTests extends ServiceContractTestSupport {
  ...
  void categoryCrudIsScopedToTheOwningShopAndUsesLogicalDeletion() {
    ...
    ItemService.CategoryView category =
        service.createCategory(
            fixture.merchantId(),
            fixture.shopId(),
            new ItemService.CreateCategoryRequest("Sides", 0));
    assertThat(service.listCategories(fixture.shopId()))
        .extracting(ItemService.CategoryView::id)
  ...
  void productCreationDefaultsOffSaleAndPublicReadsHideOffSaleProducts() {
    ...
    assertThat(
            service
                .listProducts(
                    fixture.shopId(),
                    new ItemService.ProductQuery(null, null, 1, 10, null, null, false, null))
                .items())
        .noneMatch(item -> item.id() == product.id());
    assertThat(service.getProduct(product.id(), true, fixture.merchantId()).id())
  ...
  void productUpdateChecksOwnershipAndOptimisticVersion() {
    ...
    ItemService.ProductView updated =
  ...
  void successfulReservationAndRestorationChangeStockExactlyOncePerCall() {
    ...
    List<ItemService.ProductSnapshot> snapshots =
        service.reserveForOrder(
    ...
    ItemService.ProductView product =
        service.createProduct(
            ...
            new ItemService.CreateProductRequest(
                fixture.shopId(), fixture.categoryId(), "Rice", null, new BigDecimal("12.50"), 5));
    ItemService.UpdateProductRequest update = new ItemService.UpdateProductRequest();
    update.setStatus("ON_SALE");
    update.setVersion(product.version());
    assertBusinessError(
        ApiError.IMAGE_REQUIRED,
        () -> service.updateProduct(fixture.merchantId(), product.id(), update));
  }
  ...
  private Fixture fixture(String account) {
    ...
    long shopId =
        restaurantService
            .create(merchantId, new RestaurantService.CreateRequest("Shop " + account, null))
            .id();
    long categoryId =
        service
            .createCategory(merchantId, shopId, new ItemService.CreateCategoryRequest("Meals", 0))
            .id();
