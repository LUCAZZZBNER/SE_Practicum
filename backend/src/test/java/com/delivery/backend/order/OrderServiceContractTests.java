...
class OrderServiceContractTests extends ServiceContractTestSupport {
  ...
  void createCalculatesServerTotalStoresSnapshotsAndRemovesSelectedCartItems() {
    ...
    OrderService.OrderView order =
        service.create(fixture.userId(), "order-create-key", request(fixture));
        ...
        () ->
            service.create(
                ...
                new OrderService.CreateRequest(
                    List.of(
                        new OrderService.ItemRequest(
                            fixture.cartItemId(), fixture.productVersion() + 1)),
                    fixture.addressId(),
                    null)));
  }
  ...
  void emptyCartAndChangedProductVersionFailWithoutRemovingCartOrReducingStock() {
    ...
    assertBusinessError(
        ...
        () ->
            service.create(
                fixture.userId(), "empty-key", new OrderService.CreateRequest(List.of())));
    assertBusinessError(
        ApiError.PRICE_CHANGED,
        () ->
            service.create(
                fixture.userId(),
                "changed-key",
    ...
    assertThat(shoppingService.getCart(fixture.userId()).items())
        .extracting(ShoppingService.CartItemView::id)
        .contains(fixture.cartItemId());
    assertThat(itemService.getProduct(fixture.productId(), true, fixture.merchantId()).stock())
        .isEqualTo(5);
  }
  ...
  void userAndMerchantQueriesEnforceOwnership() {
    ...
    assertThat(
            service
                .listMine(fixture.userId(), new OrderService.ListQuery(null, 1, 10, null, null))
                .items())
        .extracting(OrderService.OrderSummaryView::id)
        .contains(order.id());
    assertThat(
            service
                .listMerchantOrders(
                    fixture.merchantId(),
                    new OrderService.MerchantListQuery(fixture.shopId(), null, 1, 10, null, null))
                .items())
        .extracting(OrderService.OrderSummaryView::id)
        .contains(order.id());
    assertThat(service.getMerchantOrder(fixture.merchantId(), order.id()).id())
        .isEqualTo(order.id());
    assertBusinessError(
        ApiError.RESOURCE_NOT_FOUND, () -> service.getMine(otherUserId, order.id()));
    assertBusinessError(
        ApiError.RESOURCE_NOT_FOUND, () -> service.getMerchantOrder(otherMerchantId, order.id()));
  }
  ...
  void cancellationChangesOnlyPendingOrderAndRestoresStockOnce() {
    ...
    assertThat(itemService.getProduct(fixture.productId(), true, fixture.merchantId()).stock())
        .isEqualTo(5);
    assertBusinessError(
        ApiError.ORDER_STATE_CONFLICT, () -> service.cancel(fixture.userId(), order.id()));
    assertThat(itemService.getProduct(fixture.productId(), true, fixture.merchantId()).stock())
        .isEqualTo(5);
  }
  ...
  void suspendedMerchantsCanStillReadHistoricalOrders() {
    ...
    OrderService.OrderView order =
        service.create(fixture.userId(), "suspended-key", request(fixture));
    jdbcTemplate.update(
        "UPDATE merchants SET status = 'SUSPENDED' WHERE id = ?", fixture.merchantId());
  ...
  void nullOrderItemsProduceTheCartEmptyBusinessError() {
    ...
    assertBusinessError(
        ...
        () ->
            service.create(
                fixture.userId(), "null-items-key", new OrderService.CreateRequest(null)));
  }
  ...
  void checkoutRejectsRemarksLongerThanTwoHundredCharacters() {
    ...
    assertBusinessError(
        ApiError.VALIDATION_ERROR, () -> service.create(fixture.userId(), "remark-key", request));
  }

  @Test
  void paymentFulfillmentAndReceiptFollowTheDocumentedStateMachine() {
    Fixture fixture = fixture("order-state-machine");
    OrderService.OrderView created =
        service.create(fixture.userId(), "state-key", request(fixture));
    assertThat(service.pay(fixture.userId(), created.id(), "pay-key").status()).isEqualTo("PAID");
    assertThat(service.prepare(fixture.merchantId(), created.id(), "prepare-key").status())
        .isEqualTo("PREPARING");
    assertThat(service.deliver(fixture.merchantId(), created.id(), "deliver-key").status())
        .isEqualTo("DELIVERING");
    assertThat(service.confirmReceipt(fixture.userId(), created.id(), "receipt-key").status())
        .isEqualTo("COMPLETED");
    assertBusinessError(
        ApiError.ORDER_STATE_CONFLICT,
  ...
  void repeatedPaymentWithTheSameIdempotencyKeyReturnsTheOriginalResult() {
    ...
    OrderService.OrderView created =
        service.create(fixture.userId(), "payment-order-key", request(fixture));
  ...
  void fulfillmentActionsReplayTheSameIdempotencyKey() {
    ...
    OrderService.OrderView created =
        service.create(fixture.userId(), "create-key", request(fixture));
  ...
  private Fixture fixture(String name) {
    ...
    RestaurantService.ShopView shop =
        restaurantService.create(
            merchantId, new RestaurantService.CreateRequest("Shop " + name, null));
    ...
    ItemService.ProductView product =
        itemService.createProduct(
            ...
            new ItemService.CreateProductRequest(
                ...
                imageId,
                List.of(
                    new com.delivery.backend.item.service.SkuService.CreateRequest(
                        "默认规格", new BigDecimal("12.50"), 5))));
    ...
    long addressId =
        addressService
            ...
            .id();
    return new Fixture(
        userId, merchantId, shop.id(), product.id(), product.version(), cartItem.id(), addressId);
  }

  private UserService.UserView user(String account) {
    return userService.register(
        new UserService.RegisterRequest(
            account, "ExamplePass123!", "ExamplePass123!", "Alice", null));
  }
  ...
  private MerchantService.MerchantView merchant(String account) {
    return merchantService.register(
        new MerchantService.RegisterRequest(
            account, "ExamplePass123!", "ExamplePass123!", "Store", "13900000000"));
  }
