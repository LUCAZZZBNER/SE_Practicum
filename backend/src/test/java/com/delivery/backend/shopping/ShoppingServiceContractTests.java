...
class ShoppingServiceContractTests extends ServiceContractTestSupport {
  ...
  @Autowired private ShoppingService service;
  ...
  void addingTheSameProductCreatesOnceThenMergesQuantity() {
    ...
    ShoppingService.AddResult created =
        service.add(fixture.userId(), new ShoppingService.AddRequest(fixture.skuId(), 1));
    ShoppingService.AddResult merged =
  ...
  void quantityChangesAndDeletionAreRestrictedToTheOwningUser() {
    ...
    ShoppingService.CartItemView item =
        service.add(fixture.userId(), new ShoppingService.AddRequest(fixture.skuId(), 1)).item();

    assertThat(service.changeQuantity(fixture.userId(), item.id(), 2).quantity()).isEqualTo(2);
    assertBusinessError(
        ApiError.RESOURCE_NOT_FOUND, () -> service.changeQuantity(otherUserId, item.id(), 2));
  ...
  void checkoutLoadingReturnsOnlySelectedOwnedItemsAndSuccessfulRemovalIsSelective() {
    ...
    ShoppingService.CartItemView selected =
        service.add(fixture.userId(), new ShoppingService.AddRequest(fixture.skuId(), 1)).item();
    List<ShoppingService.CheckoutItem> checkout =
