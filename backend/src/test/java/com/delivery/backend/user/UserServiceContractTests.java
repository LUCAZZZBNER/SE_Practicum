...
class UserServiceContractTests extends ServiceContractTestSupport {
  ...
  void duplicateAccountAndPasswordMismatchAreRejectedWithoutPartialRegistration() {
    ...
    assertBusinessError(
        ApiError.ACCOUNT_EXISTS, () -> service.register(registration("duplicate-user")));
    assertBusinessError(
        ApiError.VALIDATION_ERROR,
        () ->
            service.register(
                new UserService.RegisterRequest(
                    "mismatch-user", "ExamplePass123!", "different", "Alice", null)));
  }
  ...
  void loginReturnsAUserBearerSessionAndRejectsBadCredentials() {
    ...
    UserService.AuthSession session =
        service.login(new UserService.LoginRequest("login-user", "ExamplePass123!"));
  ...
  void disabledUsersCannotUpdateTheirProfileOrAcquireAnUpdateLock() {
    ...
    assertBusinessError(
        ApiError.ACCOUNT_DISABLED, () -> service.updateCurrent(registered.id(), update));
    assertBusinessError(
        ApiError.ACCOUNT_DISABLED, () -> service.requireActiveForUpdate(registered.id()));
  }
  ...
  private static UserService.RegisterRequest registration(String account) {
    return new UserService.RegisterRequest(
